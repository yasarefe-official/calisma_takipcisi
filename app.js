...
function calculatePoints(dogru, kuranSayfa, kitapSayfa) {
    ...
    const kuranPoints = Math.floor(kuranSayfa * 0.6);
    const kitapPoints = Math.floor(kitapSayfa * 0.4);
    
    // Immediate money reward for reading
    const readingReward = Math.floor((kuranSayfa + kitapSayfa) / 4);
    money += readingReward;
...
function updateLevelUI() {
    ...
    // so I've added it from the original code.
    const LEVELS = {
